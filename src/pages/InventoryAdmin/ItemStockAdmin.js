...
const ItemStock = props => {
  ...
  async function getItemStock() {
    const { data, error } = await supabase.from("ItemStock").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setItemStock(data ?? [])
  }
  ...
  async function getItemSuppList() {
    const { data, error } = await supabase.from("ItemSupplier").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
