...
import { Table, Row, Col, Card, CardBody, CardTitle, Modal, ModalBody, Form, Label, Input, FormFeedback } from "reactstrap"
...
const Lesson = props => {
  ...
  async function getLesson() {
    const { data, error } = await supabase.from("Lesson").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setLesson(data ?? [])
  }
          ...
          >
            ...
            <div className="mb-3">
                ...
                >
                  <option value={""}>Select</option>
                    ...
                    .flatMap(el => el.sections.map((section, index) => (
            ...
            <div className="mb-3">
                ...
                <select
                  ...
                  invalid={
                    validation.touched.subjectGroup && validation.errors.subjectGroup
                      ? true
                      : false
                  }
                >
                  <option value={""}>Select</option>
                    ...
                    {Subject
                    .filter(el => el.sectionRef === validation.values.section)
                    .flatMap(el => el.subjects.map((subjects, index) => (
            ...
            <div className="mb-3">
                ...
                >
                  <option value={""}>Select</option>
                    ...
                    {Subjects
                    .filter(el => el.subjectName === validation.values.subjectGroup)
                   .map((x, index) => (
                      <option  value={x.subjectName}>{x.subjectName} ({x.subjectCode})</option>
                    ))}
                </select> 
