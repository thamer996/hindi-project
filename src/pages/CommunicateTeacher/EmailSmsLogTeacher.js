...
const EmailSmsLog = props => {
  ...
  async function getEmails() {
    const { data, error } = await supabase.from("Email").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setEmails(data ?? [])
  }
  async function getSms() {
    const { data, error } = await supabase.from("Sms").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setSms(data ?? [])
  }
  useEffect(() => {
