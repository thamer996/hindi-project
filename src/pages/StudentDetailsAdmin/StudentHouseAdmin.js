...
const StudentHouseSuper = props => {
  ...
  const validation = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required("Please Enter House name"),
    }),
    onSubmit: async values => {
      if (type === "add") {
        ...
        if (error) {
          toast.error("House Added Failed", { autoClose: 2000 })
        } else {
          toast.success("House Added", { autoClose: 2000 })
          setshow(false)
          getHouse()
        ...
        if (error) {
          toast.error("House Updated Failed", { autoClose: 2000 })
        } else {
          toast.success("House Updated", { autoClose: 2000 })
          setshow(false)
          getHouse()
          validation.resetForm()
        }
  ...
  const handleClick = () => {
    navigate("/student-add-school-house-student")
  }
  ...
  const handelDelete = async id => {
    ...
    if (error) {
      toast.error("House Deleted Failed", { autoClose: 2000 })
    } else {
      toast.success("House Deleted", { autoClose: 2000 })
      getHouse()
    }
  }
  const iconStyle = {
...
export default connect(null, { setBreadcrumbItems })(StudentHouseSuper)
