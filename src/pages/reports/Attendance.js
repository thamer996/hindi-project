...
const Attendance = props => {
  ...
  async function getStudentsAttendance() {
    const { data, error } = await supabase
      .from("StudentAttendance")
      .select("*")
      .eq("brancheId", localStorage.getItem("BranchId") ?? 1)
    const { data: dataStuds } = await supabase
      .from("Student")
      .select("*")
      .eq("brancheId", localStorage.getItem("BranchId") ?? 1)
    const res = data
      .filter(el => {
        return dataStuds.find(
          stud =>
            (stud?.rollNumber === el?.rollNumber ||
              el?.admissionNo === stud?.admissionNo) &&
            stud?.firstName === el?.name,
        )
      })
      .map(el => {
        const student = dataStuds.find(
          stud =>
            (stud?.rollNumber === el?.rollNumber ||
              el?.admissionNo === stud?.admissionNo) &&
            stud?.firstName === el?.name,
        ...
        return { ...el, ...student, id: _.uniqueId() }
      })
    setdata(res ?? [])
  }
  ...
  const handleSearch = async () => {
    const { data, error } = await supabase
      ...
      .select("*")
      .eq("brancheId", localStorage.getItem("BranchId") ?? 1)
      .ilike("attendance_date", `%${year + "-" + month + "-"}%`)
    ...
    console.log("testtttttttttttttttt", res)
  ...
  const columns = [
    ...
    },
    {
      ...
      center: true,
      minWidth: "230px",
      selector: row => row?.attendance ?? "None",
    },
    {
      ...
      center: true,
      minWidth: "230px",
      selector: row => row?.attendance_date ?? "None",
    },
    {
      name: "note",
      sortable: true,
