...
const Reference = props => {
  ...
  const [Reference, setBooks] = useState([])
  ...
  async function getCountries() {
    const { data, error } = await supabase.from("Reference").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setBooks(data ?? [])
  }
  ...
  const validation = useFormik({
    ...
    onSubmit: async values => {
      if (type === "new") {
        ...
        if (error) {
          toast.error("Reference Inserted Failed", { autoClose: 2000 })
        } else {
          toast.success("Reference Inserted", { autoClose: 2000 })
          setshow(false)
          getCountries()
        ...
        if (error) {
          toast.error("Reference Updated Failed", { autoClose: 2000 })
        } else {
          toast.success("Reference Updated", { autoClose: 2000 })
          setshow(false)
          getCountries()
          validation.resetForm()
        }
  ...
  useEffect(() => {
    props.setBreadcrumbItems("Reference", breadcrumbItems)
    getCountries()
    getVehicles()
    getRoutes()
  }, [])
  ...
  const handelDelete = async id => {
    ...
    if (error) {
      toast.error("Reference Deleted Failed", { autoClose: 2000 })
    } else {
      toast.success("Reference Deleted", { autoClose: 2000 })
      getCountries()
    }
  }
    ...
    <React.Fragment>
      ...
      <Row>
        <Col lg={12}>
          <Card>
            <CardBody>
              <CardTitle className="h4">Reference List </CardTitle>
      ...
      <Modal isOpen={show} toggle={() => setshow(!show)} centered={true}>
        <ModalBody className="py-3 px-5">
          ...
          >
            <Row>
              <div className="mb-3">
                <Label htmlFor="useremail">Reference Type</Label>
                <input
                  id="type"
                  name="type"
                  className="form-control"
...
export default connect(null, { setBreadcrumbItems })(Reference)
