...
const ComplaintType = props => {
  ...
  const [ComplaintType, setBooks] = useState([])
  ...
  async function getCountries() {
    const { data, error } = await supabase.from("ComplaintType").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setBooks(data ?? [])
  }
  ...
  const validation = useFormik({
    ...
    onSubmit: async values => {
      if (type === "new") {
        ...
        if (error) {
          toast.error("ComplaintType Inserted Failed", { autoClose: 2000 })
        } else {
          toast.success("ComplaintType Inserted", { autoClose: 2000 })
          setshow(false)
          getCountries()
        ...
        if (error) {
          toast.error("ComplaintType Updated Failed", { autoClose: 2000 })
        } else {
          toast.success("ComplaintType Updated", { autoClose: 2000 })
          setshow(false)
          getCountries()
          validation.resetForm()
        }
  ...
  useEffect(() => {
    props.setBreadcrumbItems("ComplaintType", breadcrumbItems)
    getCountries()
    getVehicles()
    getRoutes()
  }, [])
  ...
  const handelDelete = async id => {
    const { error } = await supabase.from("ComplaintType").delete().eq("id", id)

    if (error) {
      toast.error("ComplaintType Deleted Failed", { autoClose: 2000 })
    } else {
      toast.success("ComplaintType Deleted", { autoClose: 2000 })
      getCountries()
    }
  }
    ...
    <React.Fragment>
      ...
      <Row>
        <Col lg={12}>
          <Card>
            <CardBody>
              <CardTitle className="h4">ComplaintType List </CardTitle>
...
export default connect(null, { setBreadcrumbItems })(ComplaintType)
