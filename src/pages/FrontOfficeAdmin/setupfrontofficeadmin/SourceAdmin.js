...
const Source = props => {
  ...
  const [Source, setBooks] = useState([])
  ...
  async function getCountries() {
    const { data, error } = await supabase.from("Source").select("*").eq("brancheId",  localStorage.getItem("BranchId") ?? 1)
    setBooks(data ?? [])
  }
  ...
  const validation = useFormik({
    ...
    onSubmit: async values => {
      if (type === "new") {
        ...
        if (error) {
          toast.error("Source Inserted Failed", { autoClose: 2000 })
        } else {
          toast.success("Source Inserted", { autoClose: 2000 })
          setshow(false)
          getCountries()
        ...
        if (error) {
          toast.error("Source Updated Failed", { autoClose: 2000 })
        } else {
          toast.success("Source Updated", { autoClose: 2000 })
          setshow(false)
          getCountries()
          validation.resetForm()
        }
  ...
  useEffect(() => {
    props.setBreadcrumbItems("Source", breadcrumbItems)
    getCountries()
    getVehicles()
    getRoutes()
  }, [])
  ...
  const handelDelete = async id => {
    ...
    if (error) {
      toast.error("Source Deleted Failed", { autoClose: 2000 })
    } else {
      toast.success("Source Deleted", { autoClose: 2000 })
      getCountries()
    }
  }
      ...
      <Modal isOpen={show} toggle={() => setshow(!show)} centered={true}>
        <ModalBody className="py-3 px-5">
          ...
          >
            <Row>
              <div className="mb-3">
                <Label htmlFor="useremail">Source Type</Label>
                <input
                  id="type"
                  name="type"
                  className="form-control"
...
export default connect(null, { setBreadcrumbItems })(Source)
